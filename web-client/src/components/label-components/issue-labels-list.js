...
export default class extends React.Component {
    ...
    return (
      <>
        <HttpRequest
          ...
          onLoaded={(labels) => {
            const labelsProperties = EntitiesProperties.getProperties(labels, Rels.issue_label)
            return (
              <>
                <table>
                  <thead key='tableHead'>
                ...
                <Link to={ClientPaths.issueLabelAddTemplateFilled(this.props.projectName, this.props.issueId)} >
                  <button>Add Issue Label</button>
                </Link>
              </>
            )
          }}
  ...
  deleteLabel (labelName) {
    this.request = new Request(
      this.props.host,
      ApiPaths.issueLabelsUrl(this.props.issueId, labelName),
      'DELETE',
